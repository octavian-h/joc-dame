...
public class GameRepresentation {
	...
	public GameRepresentation(){
		...
		for(int i=0;i<8;i++){
			...
			for(int j=0;j<8;j++){
				if(lineColor==0)
					matrix[i][j]=EMPTY_WHITE;
				else 
					matrix[i][j]=EMPTY_BLUE;
	...
	for(int i=0;i<3;i++)
			for(int j=0;j<8;j++){
				if(matrix[i][j]!=EMPTY_WHITE)
					matrix[i][j]=BLACK;
				if(matrix[i+5][j]!=EMPTY_WHITE)
					matrix[i+5][j]=RED;
	...
	public boolean canJump(int ip, int jp,int ii,int ij,int ifin, int jfin){
		...
		if (ifin < 0 || ifin>= 8 || jfin < 0 || jfin >= 8)
            return false;  // slot  is off the board.

         if (matrix[ifin][jfin] != EMPTY_BLUE)
            return false;  // slot already contains a piece or is white.

         if (turn == RED) {
            if (matrix[ip][jp] == RED && ifin > ip)
               return false;  // Regular red piece can only move  up.
            if (matrix[ii][ij] != BLACK && matrix[ii][ij] != BLACK_KING)
               return false;  // There is no black piece to jump.
            return true;  // The jump is legal.
         }
         else {
            if (matrix[ip][jp] == BLACK && ifin < ip)
               return false;  // Regular black piece can only move down.
            if (matrix[ii][ij] != RED && matrix[ii][ij] != RED_KING)
	...
	public boolean canMove(int ip, int jp, int ifin, int jfin){
		 if (ifin < 0 || ifin >= 8 ||jfin < 0 || jfin >= 8)
	         ...
	         if (matrix[ifin][jfin] != EMPTY_BLUE)
	            return false;  // final slot  already contains a piece or is White.

	         if (turn == RED) {
	        	if (matrix[ip][jp] == RED){
	        		if(ifin > ip || Math.abs(ip-ifin)!=1)
	        			return false;// Regualr red piece can only move down.
	        	}
	        	else if(matrix[ip][jp]==RED_KING){
	        		if( Math.abs(ip-ifin)!=1)
	        			return false;
	        		}
	        	else 
	        		return false;
	...
	public boolean move(int ip, int jp, int ifin, int jfin){
		if(canMove(ip,jp,ifin,jfin)){
			if(turn==RED&&ifin==0)
				matrix[ifin][jfin]=RED_KING;
			else if(turn==BLACK&&ifin==7)
				matrix[ifin][jfin]=BLACK_KING;
			else
				matrix[ifin][jfin]=matrix[ip][jp];
	...
	private boolean canJumpFrom(int i, int j){
		if(turn==RED){
			if(canJump(i,j,i-1,j-1,i-2,j-2)||canJump(i,j,i-1,j+1,i-2,j+2))
				return true;
		}
		if(turn==BLACK){
			if(canJump(i,j,i+1,j-1,i+2,j-2)||canJump(i,j,i+1,j+1,i+2,j+2))
